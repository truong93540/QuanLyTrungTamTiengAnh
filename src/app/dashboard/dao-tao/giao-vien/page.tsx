...
export default function QuanLyGiaoVienPage() {
  ...
  // Mở rộng FormValues để chứa toàn bộ dữ liệu nâng cao của hợp đồng
  const [formValues, setFormValues] = useState({
    ...
    // Các trường dữ liệu Hợp đồng mới bổ sung
    so_hop_dong: '', 
    luong_co_ban: '', 
    ten_cong_viec: '',
    ngay_ky: '',
    ...
    tg_het_hop_dong: '',
    chi_tiet_phu_cap_text: '', // Text area thô cho người dùng nhập dạng định dạng 
  ...
  useEffect(() => { fetchGiaoViens(); }, [fetchGiaoViens]);

  // Đồng bộ hóa đổ dữ liệu DB ngược lên Giao diện Form
  const openFormModal = (teacher: GiaoVien | null = null) => {
    if (teacher) {
      setSelectedTeacher(teacher);
      const currentHd = teacher.hop_dong?.[0] || {};
      
      // Chuyển mảng JSON phụ cấp từ DB thành định dạng Text để người dùng chỉnh sửa
      let phuCapText = '';
      if (currentHd.chi_tiet_phu_cap && Array.isArray(currentHd.chi_tiet_phu_cap)) {
        phuCapText = currentHd.chi_tiet_phu_cap
          .map((item: any) => `${item.ten} - ${item.soTien}`)
          .join('\n');
      }
      ...
      setFormValues({
        ...
        // Đổ dữ liệu Hợp đồng hiện có
        so_hop_dong: currentHd.so_hop_dong || '',
        luong_co_ban: currentHd.luong_co_ban ? String(currentHd.luong_co_ban) : '',
        ten_cong_viec: currentHd.ten_cong_viec || '',
        ngay_ky: currentHd.ngay_ky ? currentHd.ngay_ky.substring(0, 10) : '',
        tg_thu_viec: currentHd.tg_thu_viec || '',
        dong_bao_hiem: currentHd.dong_bao_hiem ?? false,
        phan_tram_hoa_hong: currentHd.phan_tram_hoa_hong ? String(currentHd.phan_tram_hoa_hong) : '0',
        tg_het_hop_dong: currentHd.tg_het_hop_dong ? currentHd.tg_het_hop_dong.substring(0, 10) : '',
        chi_tiet_phu_cap_text: phuCapText,
      ...
      setFormValues({
        ...
        ngay_ky: '', tg_thu_viec: '', dong_bao_hiem: false, phan_tram_hoa_hong: '0', tg_het_hop_dong: '',
        chi_tiet_phu_cap_text: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formValues.chi_tiet_phu_cap_text.trim()) {
      const lines = formValues.chi_tiet_phu_cap_text.split('\n');
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!line) continue;
        
        if (!line.includes('-')) {
          alert(`Lỗi dòng ${i + 1} phần Phụ cấp: Thiếu ký tự gạch ngang '-' để phân tách tên và số tiền.`);
          return;
        }
        
        const parts = line.split('-');
        if (parts.length !== 2) {
          alert(`Lỗi dòng ${i + 1} phần Phụ cấp: Vui lòng chỉ dùng duy nhất một dấu '-' trên một dòng.`);
          return;
        }

        const name = parts[0].trim();
        if (!name) {
          alert(`Lỗi dòng ${i + 1} phần Phụ cấp: Tên khoản phụ cấp không được để trống.`);
          return;
        }

        // Kiểm tra từ khóa cấm
        if (name.toLowerCase() === 'chuyên cần') {
          alert(`Lỗi dòng ${i + 1} phần Phụ cấp: Từ khóa "Chuyên cần" bị cấm nhập hệ thống.`);
          return;
        }

        // CẢI TIẾN: Kiểm tra viết hoa chữ cái đầu tiên ngay trên Giao diện
        const firstChar = name.charAt(0);
        if (firstChar !== firstChar.toUpperCase() || firstChar === firstChar.toLowerCase()) {
          alert(`Lỗi dòng ${i + 1} phần Phụ cấp: Tên phụ cấp phải viết hoa chữ cái đầu tiên. Vui lòng sửa lại cụm từ "${name}".`);
          return;
        }
      }
    }
    ...
    if (formValues.tai_khoan_user) {
      payload.tai_khoan = {
        ...
        mat_khau: formValues.tai_khoan_pass || '123456@',
    ...
    // Đóng gói payload cho Hợp đồng lao động tích hợp chuỗi thô raw text phụ cấp
    if (formValues.so_hop_dong) {
      payload.hop_dong = [{
        ...
        ngay_ky: formValues.ngay_ky || null,
        tg_thu_viec: formValues.tg_thu_viec || null,
        dong_bao_hiem: formValues.dong_bao_hiem,
        phan_tram_hoa_hong: formValues.phan_tram_hoa_hong ? Number(formValues.phan_tram_hoa_hong) : 0,
        tg_het_hop_dong: formValues.tg_het_hop_dong || null,
        phu_cap_raw_text: formValues.chi_tiet_phu_cap_text // Gửi chuỗi text lên server xử lý chuyên sâu
      }];
    }
    ...
    <div className="p-6 bg-slate-50 min-h-screen text-slate-800 antialiased font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
          <div>
            <input type="text" placeholder="Tìm tên, số điện thoại, email..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full px-3 py-2 text-sm bg-slate-50 text-slate-900 placeholder-slate-400 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
          </div>
        ...
        {/* MODAL FORM: THÊM / CẬP NHẬT GIÁO VIÊN NÂNG CAO */}
        {isModalOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-xl shadow-xl border border-slate-100 max-w-4xl w-full max-h-[92vh] overflow-y-auto flex flex-col">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="text" required value={formValues.ho_ten} onChange={e => setFormValues({...formValues, ho_ten: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Ngày Sinh</label>
                      <input type="date" value={formValues.ngay_sinh} onChange={e => setFormValues({...formValues, ngay_sinh: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Giới Tính</label>
                      <select value={formValues.gioi_tinh || 'Nam'} onChange={e => setFormValues({...formValues, gioi_tinh: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                    ...
                    <div>
                      ...
                      <input type="text" value={formValues.so_dien_thoai} onChange={e => setFormValues({...formValues, so_dien_thoai: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Email <span className="text-rose-500">*</span></label>
                      <input type="email" required value={formValues.email} onChange={e => setFormValues({...formValues, email: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Địa Chỉ Cư Trú</label>
                      <input type="text" value={formValues.dia_chi} onChange={e => setFormValues({...formValues, dia_chi: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select required value={formValues.ma_chuc_vu} onChange={e => setFormValues({...formValues, ma_chuc_vu: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                        {metadata.chucVus.map(c => <option key={c.ma_chuc_vu} value={c.ma_chuc_vu}>{c.ten_chuc_vu}</option>)}
                      </select>
                    </div>
                    <div>
                      ...
                      <select required value={formValues.ma_phong_ban} onChange={e => setFormValues({...formValues, ma_phong_ban: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                        {metadata.phongBans.map(p => <option key={p.ma_phong_ban} value={p.ma_phong_ban}>{p.ten_phong_ban}</option>)}
                      </select>
                    </div>
                  </div>
                ...
                {/* 3. NÂNG CAO: THIẾT LẬP HỢP ĐỒNG LAO ĐỘNG TOÀN DIỆN */}
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <h3 className="text-xs font-bold text-amber-600 uppercase tracking-wider bg-amber-50/50 px-2.5 py-1 rounded inline-block">3. Quản Lý Chi Tiết Hợp Đồng Lao Động</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-amber-50/20 p-4 rounded-xl border border-amber-100">
                    
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Số Hợp Đồng</label>
                      <input type="text" placeholder="VD: HĐ-2026-001" value={formValues.so_hop_dong} onChange={e => setFormValues({...formValues, so_hop_dong: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Vị trí / Tên công việc</label>
                      <input type="text" placeholder="VD: Giáo viên toán cao cấp" value={formValues.ten_cong_viec} onChange={e => setFormValues({...formValues, ten_cong_viec: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Thời gian thử việc</label>
                      <input type="text" placeholder="VD: 2 tháng, 1 tháng" value={formValues.tg_thu_viec} onChange={e => setFormValues({...formValues, tg_thu_viec: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Lương Cơ Bản (VND)</label>
                      <input type="number" placeholder="Mức lương cứng" value={formValues.luong_co_ban} onChange={e => setFormValues({...formValues, luong_co_ban: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Hoa hồng (%)</label>
                      <input type="number" min="0" max="100" placeholder="0" value={formValues.phan_tram_hoa_hong} onChange={e => setFormValues({...formValues, phan_tram_hoa_hong: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Đóng Bảo Hiểm?</label>
                      <div className="flex items-center h-9">
                        <input type="checkbox" id="dong_bao_hiem" checked={formValues.dong_bao_hiem} onChange={e => setFormValues({...formValues, dong_bao_hiem: e.target.checked})} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
                        <label htmlFor="dong_bao_hiem" className="ml-2 text-sm font-medium text-slate-700 select-none">Có trích đóng bảo hiểm</label>
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Ngày ký hợp đồng</label>
                      <input type="date" value={formValues.ngay_ky} onChange={e => setFormValues({...formValues, ngay_ky: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Ngày hết hạn hợp đồng</label>
                      <input type="date" value={formValues.tg_het_hop_dong} onChange={e => setFormValues({...formValues, tg_het_hop_dong: e.target.value})} className="w-full px-3 py-2 text-sm bg-white border border-slate-200 text-slate-900 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                    </div>

                    {/* Ô NHẬP PHỤ CẤP THEO ĐỊNH DẠNG TEXTAREA */}
                    <div className="md:col-span-3">
                      <div className="flex justify-between items-center mb-1">
                        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Chi tiết Phụ cấp (Nhập theo định dạng mẫu)</label>
                      </div>
                      <textarea 
                        rows={3} 
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <input type="text" placeholder="Nơi cấp bằng" value={formValues.noi_cap} onChange={e => setFormValues({...formValues, noi_cap: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg" />
                    <input type="date" value={formValues.ngay_cap} onChange={e => setFormValues({...formValues, ngay_cap: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 text-slate-900 rounded-lg" />
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-4 border-t border-slate-100 sticky bottom-0 bg-white z-10">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 border border-slate-200 text-sm font-semibold rounded-lg text-slate-600 bg-white hover:bg-slate-50 transition-all">Hủy</button>
                  <button type="submit" className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-sm font-semibold rounded-lg text-white transition-all">Lưu lại</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {/* MODAL DIALOG: XEM CHI TIẾT GIÁO VIÊN & HỢP ĐỒNG LAO ĐỘNG TOÀN DIỆN */}
        {isDetailOpen && selectedTeacher && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl border border-slate-100 max-w-2xl w-full p-6 max-h-[88vh] overflow-y-auto flex flex-col space-y-5">
              ...
              <div className="space-y-5 text-sm text-slate-600 flex-1 overflow-y-auto pr-1">
                ...
                {/* HIỂN THỊ CHI TIẾT HỢP ĐỒNG MỞ RỘNG (GỒM PHỤ CẤP BẢN JSON) */}
                <div className="space-y-2 pt-3 border-t border-slate-100">
                  <h4 className="font-bold text-slate-800 uppercase text-xs tracking-wide text-amber-700">Hợp Đồng Lao Động Hiện Hành</h4>
                  {selectedTeacher.hop_dong && selectedTeacher.hop_dong.length > 0 ? (
                    <div className="space-y-2">
                      {selectedTeacher.hop_dong.map((h: any, idx: number) => (
                        <div key={idx} className="border-l-4 border-amber-500 pl-4 py-3 bg-amber-50/20 rounded-r-xl border border-slate-100 border-l-0 space-y-2">
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <p className="col-span-2 text-sm font-bold text-slate-900">Số HĐ: {h.so_hop_dong}</p>
                            <p><b>Công việc chính:</b> <span className="text-slate-800 font-medium">{h.ten_cong_viec || '—'}</span></p>
                            <p><b>Thời gian thử việc:</b> <span className="text-slate-800 font-medium">{h.tg_thu_viec || '—'}</span></p>
                            <p><b>Lương cơ bản:</b> <span className="text-emerald-700 font-bold">{h.luong_co_ban ? Number(h.luong_co_ban).toLocaleString('vi-VN') + ' VND' : '—'}</span></p>
                            <p><b>Phần trăm hoa hồng:</b> <span className="text-slate-800 font-semibold">{h.phan_tram_hoa_hong || 0}%</span></p>
                            <p><b>Đóng bảo hiểm:</b> <span className={`font-semibold ${h.dong_bao_hiem ? 'text-blue-600' : 'text-slate-400'}`}>{h.dong_bao_hiem ? 'Có tham gia' : 'Không'}</span></p>
                            <p><b>Ngày ký kết:</b> <span className="text-slate-800">{h.ngay_ky ? new Date(h.ngay_ky).toLocaleDateString('vi-VN') : '—'}</span></p>
                            <p className="col-span-2"><b>Thời hạn hết hợp đồng:</b> <span className="text-rose-600 font-medium">{h.tg_het_hop_dong ? new Date(h.tg_het_hop_dong).toLocaleDateString('vi-VN') : 'Vô thời hạn'}</span></p>
                          </div>
                          
                          {/* Duyệt hiển thị danh sách phụ cấp lấy từ cơ sở dữ liệu kiểu JSON */}
                          <div className="mt-2 pt-2 border-t border-slate-200/60">
                            <p className="text-xs font-bold text-slate-700 mb-1">Các khoản phụ cấp đính kèm:</p>
                            {h.chi_tiet_phu_cap && Array.isArray(h.chi_tiet_phu_cap) && h.chi_tiet_phu_cap.length > 0 ? (
                              <div className="flex flex-wrap gap-1.5">
                                {h.chi_tiet_phu_cap.map((pc: any, pIdx: number) => (
                                  <span key={pIdx} className="inline-flex text-xxs font-medium bg-white text-slate-700 border border-slate-200 px-2 py-1 rounded shadow-sm">
                                    {pc.ten}: <b className="text-blue-600 ml-1">+{Number(pc.soTien).toLocaleString('vi-VN')}đ</b>
                                  </span>
                                ))}
                              </div>
                            ) : (
                              <p className="text-xxs italic text-slate-400">Không ghi nhận khoản phụ cấp nào</p>
                            )}
                          </div>
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  <h4 className="font-bold text-slate-800 uppercase text-xs tracking-wide">Hồ Sơ Bằng Cấp Chuyên Môn</h4>
                  {selectedTeacher.ho_so_bang && selectedTeacher.ho_so_bang.length > 0 ? (
                    selectedTeacher.ho_so_bang.map((b: any, idx: number) => (
                      <div key={idx} className="border-l-4 border-blue-500 pl-3 py-1.5 bg-blue-50/30 text-xs rounded-r-lg">
                        <p className="font-bold text-slate-900">{b.bang_cap?.ten_bang_cap}</p>
                        <p className="text-slate-500 mt-0.5">Nơi cấp: {b.noi_cap || '—'} | Ngày cấp: {b.ngay_cap ? new Date(b.ngay_cap).toLocaleDateString('vi-VN') : '—'}</p>
                      </div>
                    ))
                ...
                {/* Lớp phụ trách */}
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  <h4 className="font-bold text-slate-800 uppercase text-xs tracking-wide">Lớp Phụ Trách Giảng Dạy</h4>
                  <div className="flex flex-wrap gap-1.5">
              ...
              <div className="flex justify-end pt-3 border-t border-slate-100">
                <button onClick={() => setIsDetailOpen(false)} className="px-4 py-2 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-all">Đóng</button>
              </div>
            </div>
          </div>
        )}
