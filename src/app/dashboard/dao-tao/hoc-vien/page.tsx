...
export default function HoSoHocVienPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body =
                modalMode === 'edit'
                    ? { ...formData, ma_hoc_vien: selectedHocVien?.ma_hoc_vien }
                    : formData
    ...
    const handleDelete = async () => {
        ...
        const res = await fetch(`/api/dao-tao/hoc-vien?id=${selectedHocVien.ma_hoc_vien}`, {
            method: 'DELETE',
        })
        ...
        <div className="p-6 bg-slate-50 min-h-screen text-slate-900">
            ...
            <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                ...
                <button
                    ...
                    className="bg-blue-600 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 hover:bg-blue-700 shadow-md transition font-bold">
                        ...
                        {isLoading ? (
                            ...
                            filteredData.map((hv) => (
                                <tr key={hv.ma_hoc_vien} className="hover:bg-blue-50/50 transition">
                                    ...
                                    <td className="p-4">
                                        <span
                                            className={`px-3 py-1 rounded-full text-xs font-black ${hv.trang_thai === 'Đang học' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                                            {hv.trang_thai}
                                        </span>
                                    </td>
                                    <td className="p-4">
                        ...
                        <form onSubmit={handleSubmit} className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    <h3 className="font-black text-slate-400 text-xs uppercase tracking-widest border-b pb-2">
                                        Thông tin cá nhân
                                    </h3>
                                    <div>
                                        ...
                                        <input
                                            required
                                            className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 font-bold"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            className="w-full p-2.5 bg-blue-50 border border-blue-200 rounded-lg font-black text-blue-700"
                                    ...
                                    {modalMode === 'add' && (
                                        <div
                                            className={`mt-6 p-4 rounded-xl border-2 transition-all ${isCreateReceipt ? 'border-green-500 bg-green-50' : 'border-slate-200 bg-slate-50'}`}>
                                            <label className="flex items-center gap-3 cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    className="w-5 h-5 accent-green-600"
                                                    checked={isCreateReceipt}
                                                    onChange={(e) =>
                                            ...
                                            {isCreateReceipt && (
                                                <div className="mt-4 space-y-3 animate-in fade-in slide-in-from-top-2">
                                                    <div>
                                                        ...
                                                        <input
                                                            type="number"
                                                            className="w-full p-2 border border-green-200 rounded font-black text-green-700"
                                                            value={receiptData.so_tien}
                                                            onChange={(e) =>
                                                                setReceiptData({
                                                                    ...receiptData,
                                                    ...
                                                    <div>
                                                        ...
                                                        <select
                                                            className="w-full p-2 border border-green-200 rounded font-bold"
                                                            value={receiptData.ma_nhan_su}
                                                            onChange={(e) =>
                                                                setReceiptData({
                                                                    ...receiptData,
                                                                    ma_nhan_su: e.target.value,
                                                                })
                                                            }>
                                                            <option value="">
                                                                -- Chọn nhân sự --
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            className="w-full p-2 border border-green-200 rounded text-sm italic"
                                                            value={receiptData.noi_dung}
                                                            onChange={(e) =>
                                                                setReceiptData({
                                                                    ...receiptData,
                            ...
                            <div className="mt-8 flex justify-end gap-3 pt-6 border-t">
                                <button
                                    type="button"
                                    onClick={() => setModalMode(null)}
                                    className="px-6 py-2.5 font-bold text-slate-500 hover:bg-slate-100 rounded-lg transition">
                                    Hủy bỏ
                                </button>
                                <button
                                    type="submit"
                                    className="px-10 py-2.5 bg-blue-600 text-white font-black rounded-lg hover:bg-blue-700 shadow-lg flex items-center gap-2 transition">
                                    <FaSave />{' '}
                                    {modalMode === 'add' ? 'XÁC NHẬN ĐĂNG KÝ' : 'LƯU THAY ĐỔI'}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
                        ...
                        <p className="text-slate-500 mb-6 font-medium">
                            Hành động này không thể hoàn tác. Hồ sơ học viên{' '}
                            <span className="font-bold text-slate-900">
                                {selectedHocVien?.ho_ten}
                            </span>{' '}
                            sẽ bị xóa vĩnh viễn.
                        </p>
                        <div className="flex gap-3">
                            <button
                                onClick={() => setModalMode(null)}
                                className="flex-1 py-3 font-bold text-slate-500 bg-slate-100 rounded-xl hover:bg-slate-200 transition">
                                Hủy
                            </button>
                            <button
                                onClick={handleDelete}
                                className="flex-1 py-3 font-bold text-white bg-red-600 rounded-xl hover:bg-red-700 transition shadow-lg">
                                Xóa ngay
                            </button>
                        </div>
                    </div>
