...
export default function QuanLyCamKetPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        so_buoi_vang_cho_phep: 3 as number | string, tham_gia_thi_day_du: true, so_buoi_di_muon: 3 as number | string, so_lan_thieu_bai_tap: 3 as number | string,
    ...
    const handleSaveNewStudent = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setFormErrors({}); setActiveTab('info'); setIsModalOpen(true) 
            } else {
    ...
    const selectStudent = (student: any) => { 
        setFormData({ ...formData, ten_hoc_vien: student.ho_ten, ma_hoc_vien: student.ma_hoc_vien.toString() }); 
            ...
            if (!formData.ma_hoc_vien && formData.ten_hoc_vien.trim().length >= 2) { 
                try { 
                    ...
                    if (response.ok) { 
                        ...
                        const exactMatches = data.filter((sv: any) => sv.ho_ten.toLowerCase() === formData.ten_hoc_vien.trim().toLowerCase()); 
                        if (exactMatches.length === 1) {
                            setFormData(prev => ({ ...prev, ten_hoc_vien: exactMatches[0].ho_ten, ma_hoc_vien: exactMatches[0].ma_hoc_vien.toString() })) 
                            setFormErrors(prev => ({ ...prev, ten_hoc_vien: '', ma_hoc_vien: '' }));
                        }
                    } 
                } catch (error) {} 
    ...
    const fillFormData = (row: CamKet) => {
        ...
        setViolationStats({ vang: row.so_buoi_vang_thuc_te || 0, muon: row.so_buoi_di_muon_thuc_te || 0, thieu_bt: row.so_lan_thieu_bai_tap_thuc_te || 0, bo_thi: row.da_bo_thi || false })
    }
    ...
    const openAddModal = () => {
        ...
        setViolationStats({ vang: 0, muon: 0, thieu_bt: 0, bo_thi: false }); setActiveTab('info'); setIsModalOpen(true); setFormErrors({});
    }
    ...
    const handleCheckViolation = async () => {
        if (!editingId) return; setIsChecking(true);
        try {
            const response = await fetch('/api/tuyen-sinh/cam-ket', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'check_violation', ma_cam_ket: editingId }) });
            if (response.ok) {
                const dataRes = await response.json(); const { updatedCamKet, stats } = dataRes;
                setViolationStats(stats); setFormData(prev => ({ ...prev, bi_vi_pham: updatedCamKet.bi_vi_pham, ly_do_vi_pham: updatedCamKet.ly_do_vi_pham || '' })); setData(data.map((item) => (item.ma_cam_ket === editingId ? updatedCamKet : item)));
                showToast('Phân tích thành công!', 'success');
            } else { showToast('Lỗi dữ liệu điểm danh', 'error'); }
                                            ...
                                            {isLoadingDetails ? (
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm relative overflow-hidden mb-4">
                                                        ...
                                                        >
                                                            <h3 className="font-bold text-gray-800 flex items-center gap-2 uppercase text-sm">
                                                                <FaUserGraduate className="text-blue-600 text-lg" /> Thông tin Học viên
                                                            </h3>
                                                    ...
                                                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm relative overflow-hidden mb-6">
                                                        ...
                                                        >
                                                            <h3 className="font-bold text-gray-800 flex items-center gap-2 uppercase text-sm">
                                                                <FaBookOpen className="text-green-600 text-lg" /> Khóa học áp dụng
                                                            </h3>
                                                    ...
                                                    <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                                                        ...
                                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-6 text-sm mb-5">
                                                            <div>
                                                                <span className="block text-gray-500 text-xs mb-1">Ngày ký</span>
                                                                <span className="font-semibold text-gray-800">{formData.ngay_ky ? new Date(formData.ngay_ky).toLocaleDateString('vi-VN') : '-'}</span>
                                                            </div>
                                                            <div>
                                                                <span className="block text-gray-500 text-xs mb-1">Ngày hết hạn</span>
                                                                <span className="font-semibold text-gray-800">{formData.ngay_het_han ? new Date(formData.ngay_het_han).toLocaleDateString('vi-VN') : 'Vô thời hạn'}</span>
                                                            </div>
                                                            <div className="col-span-2">
                                                                <span className="block text-gray-500 text-xs mb-1">Trạng thái</span>
                                                                <span className={`px-2.5 py-1 rounded text-xs font-bold border ${formData.trang_thai === 'Đang hiệu lực' ? 'bg-green-100 text-green-700 border-green-200' : formData.trang_thai === 'Đã hết hạn' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                                                                    {formData.trang_thai}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <div className="mb-5">
                                                            <span className="block text-gray-500 text-xs mb-1">Nội dung hợp đồng</span>
                                                        ...
                                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                                                            <div className="bg-white p-3 rounded border border-gray-200">
                                                                <span className="block text-[10px] text-gray-500 mb-1 font-bold">Vắng mặt tối đa</span>
                                                                <span className="text-lg font-black text-amber-600">{formData.so_buoi_vang_cho_phep}</span> <span className="text-xs text-gray-500">buổi</span>
                                                            </div>
                                                            <div className="bg-white p-3 rounded border border-gray-200">
                                                                <span className="block text-[10px] text-gray-500 mb-1 font-bold">Đi muộn tối đa</span>
                                                                <span className="text-lg font-black text-purple-600">{formData.so_buoi_di_muon}</span> <span className="text-xs text-gray-500">buổi</span>
                                                            </div>
                                                            <div className="bg-white p-3 rounded border border-gray-200">
                                                                <span className="block text-[10px] text-gray-500 mb-1 font-bold">Thiếu bài tập</span>
                                                                <span className="text-lg font-black text-blue-600">{formData.so_lan_thieu_bai_tap}</span> <span className="text-xs text-gray-500">lần</span>
                                                            </div>
                                                            <div className="bg-white p-3 rounded border border-gray-200 flex flex-col justify-center">
                                                                <span className="block text-[10px] text-gray-500 mb-1 font-bold">Quy định kỳ thi</span>
                                                                <span className={`text-sm font-bold ${formData.tham_gia_thi_day_du ? 'text-green-600' : 'text-gray-500'}`}>
                                                                    {formData.tham_gia_thi_day_du ? 'Bắt buộc thi' : 'Không bắt buộc'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex">
                                                            <input 
                                                                ...
                                                                value={formData.so_buoi_vang_cho_phep} 
                                                                onChange={handleChange} 
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex">
                                                            <input 
                                                                ...
                                                                value={formData.so_lan_thieu_bai_tap} 
                                                                onChange={handleChange} 
                                                ...
                                                <div>
                                                    ...
                                                    <textarea 
                                                        ...
                                                        onChange={handleChange} 
                                                        rows={3} 
                                                        className={`w-full border rounded-md px-3 py-2.5 outline-none text-black bg-white focus:ring-2 focus:ring-blue-500 resize-none ${formErrors.noi_dung_cam_ket ? 'border-red-500' : 'border-gray-300 focus:border-blue-500'}`}
                                                    ></textarea>
