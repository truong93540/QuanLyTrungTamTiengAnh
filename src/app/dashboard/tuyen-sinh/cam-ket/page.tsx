...
export default function QuanLyCamKetPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        so_buoi_vang_cho_phep: 3 as number | string, tham_gia_thi_day_du: true,
        so_buoi_di_muon: 3 as number | string, so_lan_thieu_bai_tap: 3 as number | string,
    ...
    const [newStudentData, setNewStudentData] = useState({
        ho_ten: '', ngay_sinh: '', gioi_tinh: 'Nam', so_dien_thoai: '', email: '', dia_chi: '', trang_thai: 'Đang học',
    })
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (resCamKet.ok) {
                    const dataCK = await resCamKet.json()
                    setData(Array.isArray(dataCK) ? dataCK : (dataCK.data || []))
                }
                if (resKhoaHoc.ok) {
                    const dataKH = await resKhoaHoc.json()
                    setDanhSachKhoaHoc(Array.isArray(dataKH) ? dataKH : (dataKH.data || []))
                }
                if (resLopHoc.ok) {
                    const dataLop = await resLopHoc.json();
                    setDanhSachLopHoc(Array.isArray(dataLop) ? dataLop : (dataLop.data || []));
                }
            } catch (error) {
    ...
    const handleNoStudent = () => {
        ...
        setNewStudentData({ ho_ten: '', ngay_sinh: '', gioi_tinh: 'Nam', so_dien_thoai: '', email: '', dia_chi: '', trang_thai: 'Đang học' })
        setNewStudentErrors({})
    ...
    const handleNewStudentChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (newStudentErrors[name]) setNewStudentErrors((prev) => ({ ...prev, [name]: '' }))
    }
    ...
    const handleSaveNewStudent = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setFormData({
                    ma_cam_ket: '', ngay_ky: today, ngay_het_han: '',
                    noi_dung_cam_ket: 'Cam kết tuân thủ nội quy lớp học, đi học đầy đủ và hoàn thành bài tập về nhà.',
    ...
    const handleNameChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setStudentSuggestions(Array.isArray(sugg) ? sugg : (sugg.data || []))
    ...
    const selectStudent = (student: any) => {
        ...
        setShowSuggestions(false)
        setFormErrors((prev) => ({ ...prev, ten_hoc_vien: '', ma_hoc_vien: '', general: '' }))
    }
            ...
            if (!formData.ma_hoc_vien && formData.ten_hoc_vien.trim().length >= 2) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        const exactMatches = arr.filter((sv: any) => sv.ho_ten.toLowerCase() === formData.ten_hoc_vien.trim().toLowerCase())
                        if (exactMatches.length === 1) {
                            setFormData((prev) => ({
                                ...prev, ten_hoc_vien: exactMatches[0].ho_ten, ma_hoc_vien: exactMatches[0].ma_hoc_vien.toString(),
                            }))
                            setFormErrors((prev) => ({ ...prev, ten_hoc_vien: '', ma_hoc_vien: '' }))
                        }
                    }
    ...
    const openViewModal = async (row: CamKet) => {
        ...
        try {
            const [resStudent, resCourse] = await Promise.all([
                ...
                row.ma_khoa_hoc ? fetch(`/api/tuyen-sinh/khoa-hoc?id=${row.ma_khoa_hoc}`) : Promise.resolve(null),
            ])
    ...
    const openAddModal = () => {
        ...
        setFormData({
            ma_cam_ket: '', ngay_ky: today, ngay_het_han: '',
            noi_dung_cam_ket: 'Cam kết tuân thủ nội quy lớp học, đi học đầy đủ và hoàn thành bài tập về nhà.',
            trang_thai: 'Đang hiệu lực', ma_hoc_vien: '', ten_hoc_vien: '', 
            ma_khoa_hoc: selectedCourseId ? selectedCourseId.toString() : '', 
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await fetch(`/api/tuyen-sinh/cam-ket?id=${deletingId}`, { method: 'DELETE' })
            if (response.ok) {
    ...
    const handleSaveCamKet = async () => {
        ...
        if (!formData.ngay_het_han) newErrors.ngay_het_han = 'Vui lòng chọn ngày hết hạn (Bắt buộc)'
        ...
        if (formData.ngay_het_han && formData.ngay_ky) {
            ...
            if (expirationDate <= signDate) newErrors.ngay_het_han = 'Ngày hết hạn phải lớn hơn ngày ký'
            else if (formData.trang_thai === 'Đang hiệu lực' && expirationDate < today) newErrors.ngay_het_han = 'Cam kết này đã qua ngày hết hạn so với hôm nay'
        ...
        if (formData.ma_hoc_vien && formData.ngay_ky && !newErrors.ngay_het_han) {
            const existingCamKetsOfStudent = data.filter(c => 
                ...
                c.ma_cam_ket !== editingId && 
                c.ngay_het_han
            );

            if (existingCamKetsOfStudent.length > 0) {
                existingCamKetsOfStudent.sort((a, b) => new Date(b.ngay_het_han!).getTime() - new Date(a.ngay_het_han!).getTime());
                const latestCamKet = existingCamKetsOfStudent[0];
                
                const latestEndDate = new Date(latestCamKet.ngay_het_han!);
                const newStartDate = new Date(formData.ngay_ky);
                
                latestEndDate.setHours(0,0,0,0);
                newStartDate.setHours(0,0,0,0);

                if (newStartDate <= latestEndDate) {
                    const tenKhoaHocDangHoc = latestCamKet.khoa_hoc?.ten_khoa_hoc || 'một khóa học khác';
                    newErrors.general = `Học viên này đang tham gia khóa "${tenKhoaHocDangHoc}" (đến ngày ${latestEndDate.toLocaleDateString('vi-VN')}). Không thể đăng ký mới lúc này.`;
                    showToast('Học viên đang trong thời gian học khóa khác!', 'error');
                }
            }
        }
    ...
    const baseFilteredData = useMemo(() => {
        return data.filter(item => {
            ...
            return (item.hoc_vien?.ho_ten || '').toLowerCase().includes(lowerSearch) || 
    ...
    const classesInSelectedCourse = useMemo(() => {
        ...
        if (tuKhoaLopHoc.trim() !== '') {
            classes = classes.filter(lop => lop.ten_lop.toLowerCase().includes(tuKhoaLopHoc.toLowerCase()));
        }
        return classes;
    ...
    const unassignedStudentsCount = useMemo(() => {
        return courseFilteredData.filter(c => {
            const hasClassInCourse = c.hoc_vien?.tham_gia_lop?.some(tl => 
                danhSachLopHoc.filter(l => Number(l.ma_khoa_hoc) === Number(selectedCourseId)).some(lop => lop.ma_lop_hoc === tl.lop_hoc?.ma_lop_hoc)
            );
            return !hasClassInCourse;
        }).length;
    ...
    const renderTableRow = (row: CamKet, index: number) => (
        <tr key={row.ma_cam_ket} className={`border-b hover:bg-blue-50 transition ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
            ...
            <td className="px-4 py-4 font-medium text-gray-800">
                {row.khoa_hoc?.ten_khoa_hoc || <span className="text-gray-400 italic">Chưa liên kết</span>}
            </td>
            <td className="px-4 py-4 text-center text-gray-600">{formatDateForView(row.ngay_ky)}</td>
            <td className="px-4 py-4 text-center">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap border inline-block ${row.trang_thai === 'Đang hiệu lực' ? 'bg-green-100 text-green-700 border-green-200' : row.trang_thai === 'Đã hết hạn' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                    {row.trang_thai}
                </span>
            </td>
            <td className="px-4 py-4 text-center">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="relative w-full md:w-80">
                ...
                <input
                    ...
                    value={tuKhoaKhoaHoc}
                    onChange={(e) => setTuKhoaKhoaHoc(e.target.value)}
                    className="block w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 shadow-sm focus:border-[#1d4ed8] focus:outline-none focus:ring-4 focus:ring-blue-500/10 transition-all"
                />
            </div>
        </div>
        ...
        >
            ...
            <span className="font-bold text-center">Tất cả khóa học</span>
                ...
                {filteredKhoaHoc.length === 0 ? (
                    ...
                    filteredKhoaHoc.map(kh => {
                        const soLuongCamKet = data.filter(c => c.ma_khoa_hoc === kh.ma_khoa_hoc).length;
                        return (
                            <div 
                                key={kh.ma_khoa_hoc}
                            ...
                            >
                                ...
                                <h3 className="font-bold text-gray-800 text-sm line-clamp-2 pr-16">{kh.ten_khoa_hoc}</h3>
        ...
        <div className="overflow-x-auto rounded-t-lg border border-gray-200">
            <table className="w-full text-sm text-left">
                ...
                <tbody className="text-gray-700">
                    {isLoading ? <tr><td colSpan={7} className="text-center py-8">Đang tải...</td></tr> : 
                     currentItems.length === 0 ? <tr><td colSpan={7} className="text-center py-8">Không có dữ liệu</td></tr> : 
                ...
                {selectedCourseId && !selectedClassId && (
                    <div className="animate-fade-in-right">
                        <div className="flex items-center gap-4 bg-gray-50 border border-gray-200 p-3 rounded-lg mb-6">
                            ...
                            <div>
                                <span className="text-xs text-gray-500 font-bold uppercase tracking-wider block">Khóa học đang chọn:</span>
                                <span className="text-lg font-bold text-[#1d4ed8]">
                                    {danhSachKhoaHoc.find(k => k.ma_khoa_hoc === selectedCourseId)?.ten_khoa_hoc}
                                </span>
                            </div>
                        </div>

                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                            <h2 className="text-lg font-bold text-gray-800">Danh Sách Lớp Học:</h2>
                            <div className="relative w-full md:w-80">
                                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                    <FaSearch className="text-gray-400 text-sm" />
                                </div>
                                <input
                                    ...
                                    value={tuKhoaLopHoc}
                                    onChange={(e) => setTuKhoaLopHoc(e.target.value)}
                                    className="block w-full rounded-md border border-gray-300 bg-white py-2 pl-9 pr-3 text-sm text-gray-900 shadow-sm focus:border-[#1d4ed8] focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                                />
                            </div>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 max-h-[350px] overflow-y-auto pr-2 custom-scroll mb-4">
                            {tuKhoaLopHoc.trim() === '' && (
                                <div 
                                ...
                                >
                                    ...
                                    <h3 className="font-bold text-amber-600 text-sm mb-2 line-clamp-1 ml-2" title="Chưa xếp lớp">Chưa xếp lớp</h3>
                                    <div className="bg-gray-100 text-gray-700 text-xs font-semibold px-2 py-1 rounded inline-block ml-2">
                                        {unassignedStudentsCount} hồ sơ
                                    </div>
                                </div>
                            )}
                            ...
                            {classesInSelectedCourse.length === 0 ? (
                                ...
                                classesInSelectedCourse.map(lop => {
                                    const count = data.filter(c => {
                                        const isRightCourse = Number(c.ma_khoa_hoc) === Number(selectedCourseId);
                                        const isStudentInClass = c.hoc_vien?.tham_gia_lop?.some(tl => tl.lop_hoc?.ma_lop_hoc === lop.ma_lop_hoc);
                                        return isRightCourse && isStudentInClass;
                                    }).length;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="bg-gray-100 text-gray-700 text-xs font-semibold px-2 py-1 rounded inline-block">
                                                {count} hồ sơ
                                            </div>
                                        </div>
                                    )
                ...
                {selectedCourseId && selectedClassId && (
                    <div className="animate-fade-in-right">
                        <div className="flex items-center gap-4 bg-gray-50 border border-gray-200 p-3 rounded-lg mb-6">
                            ...
                            <div>
                                <span className="text-xs text-gray-500 font-bold uppercase tracking-wider block">Lớp học đang xem:</span>
                                <span className="text-lg font-bold text-[#1d4ed8]">
                                    {selectedClassId === -1 ? 'Học viên chưa xếp lớp' : danhSachLopHoc.find(l => l.ma_lop_hoc === selectedClassId)?.ten_lop}
                                </span>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
                    <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl animate-fade-in-up flex flex-col border border-gray-200">
                        ...
                        <div className="p-6 space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="ho_ten" value={newStudentData.ho_ten} onChange={handleNewStudentChange} className={`w-full border rounded-md p-2.5 outline-none text-black ${newStudentErrors.ho_ten ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-gray-300 focus:ring-2 focus:ring-blue-500'}`} />
                                    {newStudentErrors.ho_ten && <p className="text-red-500 text-xs italic mt-1 font-medium">{newStudentErrors.ho_ten}</p>}
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Ngày sinh <span className="text-red-500">*</span></label>
                                    <input type="date" name="ngay_sinh" value={newStudentData.ngay_sinh} onChange={handleNewStudentChange} className={`w-full border rounded-md p-2.5 outline-none text-black ${newStudentErrors.ngay_sinh ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-gray-300 focus:ring-2 focus:ring-blue-500'}`} />
                                    {newStudentErrors.ngay_sinh && <p className="text-red-500 text-xs italic mt-1 font-medium">{newStudentErrors.ngay_sinh}</p>}
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Giới tính</label>
                                    <select name="gioi_tinh" value={newStudentData.gioi_tinh} onChange={handleNewStudentChange} className="w-full border border-gray-300 rounded-md p-2.5 outline-none bg-white text-black">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="so_dien_thoai" value={newStudentData.so_dien_thoai} onChange={handleNewStudentChange} className="w-full border border-gray-300 rounded-md p-2.5 outline-none text-black" />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Email</label>
                                    <input type="email" name="email" value={newStudentData.email} onChange={handleNewStudentChange} placeholder="ví dụ: nguyenvana@gmail.com" className="w-full border border-gray-300 rounded-md p-2.5 outline-none text-black" />
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Địa chỉ</label>
                                    <input type="text" name="dia_chi" value={newStudentData.dia_chi} onChange={handleNewStudentChange} className="w-full border border-gray-300 rounded-md p-2.5 outline-none text-black" />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Trạng thái</label>
                                    <select name="trang_thai" value={newStudentData.trang_thai} onChange={handleNewStudentChange} className="w-full border border-gray-300 rounded-md p-2.5 outline-none bg-white text-black">
                        ...
                        <div className="p-5 border-t border-gray-200 bg-gray-50 rounded-b-lg flex justify-end gap-3">
                            {newStudentErrors.general && <div className="mr-auto text-red-600 font-medium self-center text-sm">{newStudentErrors.general}</div>}
                            ...
                            <button onClick={handleSaveNewStudent} disabled={isSubmittingStudent} className="flex items-center gap-2 px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium transition disabled:opacity-50">
                                {isSubmittingStudent ? <FaSpinner className="animate-spin" /> : <FaSave />} Lưu và Tiếp tục
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-lg w-full max-w-4xl shadow-2xl animate-fade-in-up flex flex-col max-h-[95vh] overflow-hidden border border-gray-200">
                        <div className="flex justify-between items-center p-5 border-b border-gray-200 bg-white rounded-t-lg shrink-0">
                            <h2 className="text-xl font-bold text-[#1d4ed8] flex items-center gap-2 uppercase tracking-wide">
                                <FaShieldAlt /> {isViewMode ? 'Hồ Sơ Cam Kết' : editingId ? 'Cập Nhật Bản Cam Kết' : 'Tạo Bản Cam Kết Mới'}
                            </h2>
                        ...
                        <div className="flex px-6 pt-2 bg-gray-50 border-b border-gray-200 shrink-0">
                            <button onClick={() => setActiveTab('info')} className={`py-2 px-4 font-bold border-b-2 outline-none transition-colors ${activeTab === 'info' ? 'border-[#1d4ed8] text-[#1d4ed8]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Hồ sơ & Điều khoản</button>
                            {editingId && <button onClick={() => setActiveTab('violation')} className={`py-2 px-4 font-bold border-b-2 outline-none transition-colors ${activeTab === 'violation' ? 'border-[#1d4ed8] text-[#1d4ed8]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Tra cứu Vi phạm</button>}
                        </div>
                                                ...
                                                <div className="space-y-4">
                                                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm relative overflow-hidden mb-4">
                                                        ...
                                                        <div className="flex justify-between items-center p-4 pl-6 cursor-pointer hover:bg-blue-50/30 transition-colors" onClick={() => setShowStudentInfo(!showStudentInfo)}>
                                                            <h3 className="font-bold text-gray-800 flex items-center gap-2 uppercase text-sm"><FaUserGraduate className="text-blue-600 text-lg" /> Thông tin Học viên</h3>
                                                            <button className="flex items-center gap-1 bg-blue-50 text-blue-700 px-3 py-1.5 rounded text-xs font-semibold hover:bg-blue-100 transition pointer-events-none">
                                                                {showStudentInfo ? <FaChevronUp /> : <FaChevronDown />} {showStudentInfo ? 'Thu gọn' : 'Xem chi tiết'}
                                                            </button>
                                                        </div>
                                                        {showStudentInfo && (
                                                            <div className="p-5 pl-6 border-t border-gray-100 animate-fade-in-down bg-white">
                                                                <h3 className="text-sm font-bold text-blue-800 uppercase mb-4 border-b border-gray-100 pb-2 flex items-center gap-2"><FaUserGraduate /> Hồ sơ cá nhân</h3>
                                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-5 gap-x-6 text-sm">
                                                                    <div><span className="block text-gray-500 text-xs mb-1">Mã Học Viên</span><span className="font-bold text-gray-800 bg-gray-100 px-2 py-0.5 rounded">{fullStudentInfo?.ma_hoc_vien || formData.ma_hoc_vien}</span></div>
                                                                    <div><span className="block text-gray-500 text-xs mb-1">Họ và tên</span><span className="font-bold text-blue-700 text-base">{fullStudentInfo?.ho_ten || formData.ten_hoc_vien}</span></div>
                                                    ...
                                                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm relative overflow-hidden mb-6">
                                                        ...
                                                        <div className="flex justify-between items-center p-4 pl-6 cursor-pointer hover:bg-green-50/30 transition-colors" onClick={() => setShowCourseInfo(!showCourseInfo)}>
                                                            <h3 className="font-bold text-gray-800 flex items-center gap-2 uppercase text-sm"><FaBookOpen className="text-green-600 text-lg" /> Khóa học áp dụng</h3>
                                                            <button className="flex items-center gap-1 bg-green-50 text-green-700 px-3 py-1.5 rounded text-xs font-semibold hover:bg-green-100 transition pointer-events-none">
                                                                {showCourseInfo ? <FaChevronUp /> : <FaChevronDown />} {showCourseInfo ? 'Thu gọn' : 'Xem chi tiết'}
                                                            </button>
                                                        </div>
                                                        {showCourseInfo && (
                                                            <div className="p-5 pl-6 border-t border-gray-100 animate-fade-in-down bg-white">
                                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-5 gap-x-6 text-sm">
                                                                    <div className="col-span-2"><span className="block text-gray-500 text-xs mb-1">Tên khóa học</span><span className="font-bold text-gray-900 text-base">{fullCourseInfo?.ten_khoa_hoc || danhSachKhoaHoc.find((k) => k.ma_khoa_hoc.toString() === formData.ma_khoa_hoc)?.ten_khoa_hoc || '-'}</span></div>
                                                                    <div><span className="block text-gray-500 text-xs mb-1">Học phí</span><span className="font-bold text-red-600 text-base">{formatCurrency(fullCourseInfo?.hoc_phi)}</span></div>
                                                                    <div><span className="block text-gray-500 text-xs mb-1">Thời lượng</span><span className="font-semibold text-gray-800 text-base">{fullCourseInfo?.thoi_luong || 'Chưa cập nhật'}</span></div>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm relative overflow-hidden mb-6">
                                                        ...
                                                        <div className="flex justify-between items-center p-4 pl-6 border-b border-gray-100 bg-white">
                                                            <h3 className="font-bold text-gray-800 flex items-center gap-2 uppercase text-sm"><FaFileAlt className="text-purple-600 text-lg" /> Chi tiết Bản cam kết</h3>
                                                        </div>
                                                        <div className="p-5 pl-6 bg-white animate-fade-in-down">
                                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-y-5 gap-x-6 text-sm mb-6">
                                                                <div><span className="block text-gray-500 text-xs mb-1">Ngày ký</span><span className="font-semibold text-gray-800 text-base">{formData.ngay_ky ? new Date(formData.ngay_ky).toLocaleDateString('vi-VN') : '-'}</span></div>
                                                                <div><span className="block text-gray-500 text-xs mb-1">Ngày hết hạn</span><span className="font-semibold text-gray-800 text-base">{formData.ngay_het_han ? new Date(formData.ngay_het_han).toLocaleDateString('vi-VN') : 'Vô thời hạn'}</span></div>
                                                                <div className="col-span-2"><span className="block text-gray-500 text-xs mb-1">Trạng thái</span><span className={`px-2.5 py-1 rounded text-xs font-bold border inline-block ${formData.trang_thai === 'Đang hiệu lực' ? 'bg-green-100 text-green-700 border-green-200' : formData.trang_thai === 'Đã hết hạn' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-red-100 text-red-700 border-red-200'}`}>{formData.trang_thai}</span></div>
                                                            </div>
                                                            <div className="mb-6">
                                                                <span className="block text-gray-500 text-xs mb-2 font-bold uppercase">Nội dung hợp đồng</span>
                                                                <div className="bg-gray-50 p-4 border border-gray-200 rounded-md text-gray-700 leading-relaxed min-h-[60px]">{formData.noi_dung_cam_ket}</div>
                                                            </div>
                                                            <h4 className="text-xs font-bold text-gray-500 uppercase mb-3">Điều kiện nội quy bắt buộc</h4>
                                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                                                                <div className="bg-gray-50 p-3 rounded-md border border-gray-200"><span className="block text-[10px] text-gray-500 mb-1 font-bold">Vắng mặt tối đa</span><span className="text-lg font-black text-amber-600">{formData.so_buoi_vang_cho_phep}</span> <span className="text-xs text-gray-500">buổi</span></div>
                                                                <div className="bg-gray-50 p-3 rounded-md border border-gray-200"><span className="block text-[10px] text-gray-500 mb-1 font-bold">Đi muộn tối đa</span><span className="text-lg font-black text-purple-600">{formData.so_buoi_di_muon}</span> <span className="text-xs text-gray-500">buổi</span></div>
                                                                <div className="bg-gray-50 p-3 rounded-md border border-gray-200"><span className="block text-[10px] text-gray-500 mb-1 font-bold">Thiếu bài tập</span><span className="text-lg font-black text-blue-600">{formData.so_lan_thieu_bai_tap}</span> <span className="text-xs text-gray-500">lần</span></div>
                                                                <div className="bg-gray-50 p-3 rounded-md border border-gray-200 flex flex-col justify-center"><span className="block text-[10px] text-gray-500 mb-1 font-bold">Quy định kỳ thi</span><span className={`text-sm font-bold ${formData.tham_gia_thi_day_du ? 'text-green-600' : 'text-gray-500'}`}>{formData.tham_gia_thi_day_du ? 'Bắt buộc thi' : 'Không bắt buộc'}</span></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="space-y-6">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                <div className="relative">
                                                    ...
                                                    <input type="text" name="ten_hoc_vien" value={formData.ten_hoc_vien} onChange={handleNameChange} onFocus={() => studentSuggestions.length > 0 && setShowSuggestions(true)} onBlur={handleNameBlur} placeholder="Nhập tên học viên..." className={`w-full border rounded-md px-3 py-2.5 outline-none text-black bg-white focus:ring-2 focus:ring-blue-500 ${formErrors.ten_hoc_vien ? 'border-red-500' : 'border-gray-300 focus:border-blue-500'}`} />
                                                    {formErrors.ten_hoc_vien && <p className="text-red-500 text-xs italic mt-1 font-medium">{formErrors.ten_hoc_vien}</p>}
                                                    {showSuggestions && (
                                                        <ul className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                                            {isSearching ? <li className="p-3 text-sm text-gray-500 text-center">Đang tìm kiếm...</li> : studentSuggestions.length > 0 ? (
                                                                studentSuggestions.map((sv) => (
                                                                    <li key={sv.ma_hoc_vien} onMouseDown={() => selectStudent(sv)} className="p-3 hover:bg-blue-50 cursor-pointer border-b border-gray-100 last:border-0">
                                                                        <div className="font-bold text-gray-800">{sv.ho_ten}</div><div className="text-xs text-gray-500 mt-1">Mã HV: <span className="text-blue-600 font-semibold">{sv.ma_hoc_vien}</span></div>
                                                                    </li>
                                                                ))
                                                ...
                                                <div>
                                                    ...
                                                    <input type="number" name="ma_hoc_vien" value={formData.ma_hoc_vien} onChange={handleChange} onBlur={handleIdBlur} className={`w-full border rounded-md px-3 py-2.5 outline-none text-black bg-white focus:ring-2 focus:ring-blue-500 ${formErrors.ma_hoc_vien ? 'border-red-500' : 'border-gray-300 focus:border-blue-500'}`} />
                                                    {formErrors.ma_hoc_vien && <p className="text-red-500 text-xs italic mt-1 font-medium">{formErrors.ma_hoc_vien}</p>}
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                <div>
                                                    ...
                                                    <select 
                                                        ...
                                                        className={`w-full border rounded-md px-3 py-2.5 outline-none text-black focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed ${formErrors.ma_khoa_hoc ? 'border-red-500' : 'border-gray-300 focus:border-blue-500 bg-white'}`}
                                                    >
                                                        <option value="">-- Chọn khóa học --</option>
                                                        {danhSachKhoaHoc.map((kh) => <option key={kh.ma_khoa_hoc} value={kh.ma_khoa_hoc.toString()}>{kh.ten_khoa_hoc}</option>)}
                                                    </select>
                                                    {formErrors.ma_khoa_hoc && <p className="text-red-500 text-xs italic mt-1 font-medium">{formErrors.ma_khoa_hoc}</p>}
                                                </div>
                                                <div>
                                                    <label className="block text-[14px] font-medium text-gray-700 mb-1.5">Trạng thái</label>
                                                    <select name="trang_thai" value={formData.trang_thai} onChange={handleChange} className="w-full border border-gray-300 rounded-md px-3 py-2.5 outline-none text-black focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                <div>
                                                    <label className="block text-[14px] font-medium text-gray-700 mb-1.5">Ngày ký <span className="text-red-500">*</span></label>
                                                    <input 
                                                        type="date" 
                                                        name="ngay_ky" 
                                                        ...
                                                        onChange={handleChange} 
                                                        disabled
                                                        className="w-full border rounded-md px-3 py-2.5 outline-none text-gray-500 bg-gray-100 cursor-not-allowed border-gray-300" 
                                                    />
                                                    {formErrors.ngay_ky && <p className="text-red-500 text-xs italic mt-1 font-medium">{formErrors.ngay_ky}</p>}
                                                </div>
                                                <div>
                                                    <label className="block text-[14px] font-medium text-gray-700 mb-1.5">Ngày hết hạn <span className="text-red-500">*</span></label>
                                                    <input 
                                                        type="date" 
                                                        name="ngay_het_han" 
                                            ...
                                            <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
                                                    <div>
                                                        ...
                                                        <div className="flex">
                                                            <input type="number" name="so_buoi_vang_cho_phep" value={formData.so_buoi_vang_cho_phep} onChange={handleChange} className={`w-16 border rounded-l px-2 py-2 text-center text-blue-600 font-bold text-[15px] outline-none ${formErrors.so_buoi_vang_cho_phep ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:border-gray-400'}`} />
                                                        ...
                                                        {formErrors.so_buoi_vang_cho_phep && <p className="text-red-500 text-[10px] mt-1 font-medium">{formErrors.so_buoi_vang_cho_phep}</p>}
                                                    </div>
                                                    <div>
                                                        <label className="block text-xs text-gray-500 mb-1 font-bold">Tối đa ĐI MUỘN</label>
                                                        <div className="flex">
                                                            <input type="number" name="so_buoi_di_muon" value={formData.so_buoi_di_muon} onChange={handleChange} className={`w-16 border rounded-l px-2 py-2 text-center text-blue-600 font-bold text-[15px] outline-none ${formErrors.so_buoi_di_muon ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:border-gray-400'}`} />
                                                            <span className="bg-gray-50 border border-l-0 border-gray-300 rounded-r px-3 py-2 text-sm text-gray-600 font-medium">buổi</span>
                                                        </div>
                                                        {formErrors.so_buoi_di_muon && <p className="text-red-500 text-[10px] mt-1 font-medium">{formErrors.so_buoi_di_muon}</p>}
                                                    </div>
                                                    <div>
                                                        <label className="block text-xs text-gray-500 mb-1 font-bold">Thiếu BÀI TẬP</label>
                                                        <div className="flex">
                                                            <input type="number" name="so_lan_thieu_bai_tap" value={formData.so_lan_thieu_bai_tap} onChange={handleChange} className={`w-16 border rounded-l px-2 py-2 text-center text-blue-600 font-bold text-[15px] outline-none ${formErrors.so_lan_thieu_bai_tap ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:border-gray-400'}`} />
                                                        ...
                                                        {formErrors.so_lan_thieu_bai_tap && <p className="text-red-500 text-[10px] mt-1 font-medium">{formErrors.so_lan_thieu_bai_tap}</p>}
                                                    </div>
                                                    <div className="flex flex-col justify-center pt-3">
                                                        <label className="flex items-center cursor-pointer">
                                                            <input type="checkbox" name="tham_gia_thi_day_du" checked={formData.tham_gia_thi_day_du} onChange={handleChange} className="w-4 h-4 text-blue-600 rounded border-gray-300" />
                                                            <span className="ml-2 text-xs font-bold text-gray-600 uppercase">Bắt buộc thi</span>
                                                        </label>
                                                    </div>
                                                </div>
                                                <div>
                                                    <label className="block text-[14px] font-medium text-gray-700 mb-1.5 uppercase">Nội dung hợp đồng chi tiết <span className="text-red-500">*</span></label>
                                                    <textarea name="noi_dung_cam_ket" value={formData.noi_dung_cam_ket} onChange={handleChange} rows={3} className={`w-full border rounded-md px-3 py-2.5 outline-none text-black bg-white focus:ring-2 focus:ring-blue-500 resize-none ${formErrors.noi_dung_cam_ket ? 'border-red-500' : 'border-gray-300 focus:border-blue-500'}`}></textarea>
                                                    {formErrors.noi_dung_cam_ket && <p className="text-red-500 text-xs italic mt-1 font-medium">{formErrors.noi_dung_cam_ket}</p>}
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-6 animate-fade-in-up">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col items-center p-5 relative overflow-hidden">
                                            <div className={`absolute top-0 left-0 w-full h-1.5 ${violationStats.thieu_bt > (Number(formData.so_lan_thieu_bai_tap) || 0) ? 'bg-blue-500' : 'bg-blue-500'}`}></div>
                                            ...
                                            <div className="flex items-baseline gap-1"><span className="text-4xl font-black text-gray-800">{violationStats.thieu_bt}</span><span className="text-sm font-medium text-gray-500">lần</span></div>
                                        ...
                                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col items-center p-5 relative overflow-hidden">
                                            <div className={`absolute top-0 left-0 w-full h-1.5 ${violationStats.vang > (Number(formData.so_buoi_vang_cho_phep) || 0) ? 'bg-amber-500' : 'bg-amber-500'}`}></div>
                                            ...
                                            <div className="flex items-baseline gap-1"><span className="text-4xl font-black text-gray-800">{violationStats.vang}</span><span className="text-sm font-medium text-gray-500">buổi</span></div>
                                        ...
                                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col items-center p-5 relative overflow-hidden">
                                            <div className={`absolute top-0 left-0 w-full h-1.5 ${violationStats.muon > (Number(formData.so_buoi_di_muon) || 0) ? 'bg-purple-500' : 'bg-purple-500'}`}></div>
                                            ...
                                            <div className="flex items-baseline gap-1"><span className="text-4xl font-black text-gray-800">{violationStats.muon}</span><span className="text-sm font-medium text-gray-500">buổi</span></div>
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative animate-fade-in-up">
                        ...
                        <p className="text-gray-600 mb-6 ml-16 text-sm">Bạn có chắc chắn muốn xóa bản cam kết này?</p>
